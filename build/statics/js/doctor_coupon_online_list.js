...
/*****************************************************source：resources/js/business/doctor/doctor_coupon_online_list.js*****************************************************/params.cityName = decodeURIComponent(params.cityName || '全国');
lyb.parse();
...
var renderList = renderClinicDoctorList;
if (params.type === 'online') {
    renderList = renderOnlineDoctorList
}
var pull = lyb.pullUpLoading({
    el: '#wrapper',
    pageSize: 10,
    url: ctx + 'doctor/info/getDoctorByCouponId',
    params: {city: (params.cityName || '').replace(/全国/g, ''), condition: searchBox.getValue(), couponId: params.couponId, flag: params.type === 'online' ? 'netAll' : 'clinic'},
    success: function (result) {
